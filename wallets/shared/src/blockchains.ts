...
/** Cosmos is absent: it has no namespace module, we don't connect to it via the hub. */
export const CAIP_NAMESPACES = {
  EVM: CAIP_EVM_NAMESPACE,
  SOLANA: CAIP_SOLANA_NAMESPACE,
  STARKNET: CAIP_STARKNET_NAMESPACE,
  STELLAR: CAIP_STELLAR_NAMESPACE,
  SUI: CAIP_SUI_NAMESPACE,
  TON: CAIP_TVM_NAMESPACE,
  TRON: CAIP_TRON_NAMESPACE,
  UTXO: CAIP_UTXO_NAMESPACE,
  XRPL: CAIP_XRPL_NAMESPACE,
} as const;

/** Pair a namespace with a reference here, not at call sites — that's where ids get crossed. */
export const CAIP_CHAINS = {
  BITCOIN: `${CAIP_NAMESPACES.UTXO}:${CAIP_BITCOIN_CHAIN_ID}`,
  BITCOINCASH: `${CAIP_NAMESPACES.UTXO}:${CAIP_BITCOINCASH_CHAIN_ID}`,
  DOGECOIN: `${CAIP_NAMESPACES.UTXO}:${CAIP_DOGECOIN_CHAIN_ID}`,
  ETHEREUM: `${CAIP_NAMESPACES.EVM}:${CAIP_ETHEREUM_CHAIN_ID}`,
  LITECOIN: `${CAIP_NAMESPACES.UTXO}:${CAIP_LITECOIN_CHAIN_ID}`,
  SOLANA: `${CAIP_NAMESPACES.SOLANA}:${CAIP_SOLANA_CHAIN_ID}`,
  STARKNET: `${CAIP_NAMESPACES.STARKNET}:${CAIP_STARKNET_CHAIN_ID}`,
  STELLAR: `${CAIP_NAMESPACES.STELLAR}:${CAIP_STELLAR_CHAIN_ID}`,
  SUI: `${CAIP_NAMESPACES.SUI}:${CAIP_SUI_CHAIN_ID}`,
  TON: `${CAIP_NAMESPACES.TON}:${CAIP_TON_CHAIN_ID}`,
  TRON: `${CAIP_NAMESPACES.TRON}:${CAIP_TRON_CHAIN_ID}`,
  XRPL: `${CAIP_NAMESPACES.XRPL}:${CAIP_XRPL_CHAIN_ID}`,
  ZCASH: `${CAIP_NAMESPACES.UTXO}:${CAIP_ZCASH_CHAIN_ID}`,
...
const CAIP_BY_TRANSACTION_TYPE: Record<TransactionType, CaipResolver> = {
  ...
  [TransactionType.EVM]: (blockchainMeta) =>
    toNumericCaip(CAIP_NAMESPACES.EVM, blockchainMeta.chainId),
